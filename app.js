...
function formationElementCount(total) {
  return Math.min(4, Math.ceil(Math.sqrt(total)));
}
...
function drillPerformanceVideos(guideId) {
  const videos = DRILL_PERFORMANCE_VIDEOS[guideId] || [];
  if (!videos.length) return "";
  return `<section class="drill-performance"><div class="drill-performance-heading"><div><p class="eyebrow">More cadets in motion</p><h3>Watch the movements performed</h3></div><p>Use these wider, full-body demonstrations to see posture, timing, cadence, and how the formation moves together.</p></div><div class="drill-performance-grid">${videos.map((video) => `<article class="drill-video drill-video-extra" data-video-id="${video.id}" data-start="0"><div class="drill-video-preview"><img src="https://i.ytimg.com/vi/${video.id}/hqdefault.jpg" alt="${video.title} video demonstration" loading="lazy"><button type="button" data-play-drill-video aria-label="Play ${video.title}"><span aria-hidden="true">▶</span> Watch cadets perform</button></div><div class="drill-video-meta"><strong>${video.title}</strong><span>${video.creator}</span><p>${video.note}</p><a href="https://www.youtube.com/watch?v=${video.id}" target="_blank" rel="noopener">Open on YouTube ↗</a></div></article>`).join("")}</div><p class="drill-performance-caution"><strong>Practice note:</strong> The AFROTC videos are supplemental Air Force-style demonstrations. For your CAP test, follow the current CAPP 60-33 standard and your CAP instructor.</p></section>`;
}
...
function initDrillVideos() {
  document.querySelectorAll("[data-play-drill-video]").forEach((button) => button.addEventListener("click", () => {
    ...
    iframe.src = `https://www.youtube-nocookie.com/embed/${videoId}?start=${start || 0}${endParam}&autoplay=1&rel=0`;
