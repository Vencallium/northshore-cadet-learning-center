...
function drillVisualSequence(key) {
  ...
  return `<figure class="drill-sequence"><figcaption>Movement from above · ${frames.length} snapshots</figcaption><div class="drill-frames">${frames.map(([stage, pose, cue], i) => `<div class="drill-frame"><strong><span>${String(i + 1).padStart(2, "0")}</span> ${stage}</strong>${drillSceneSvg(pose, `${stage}. ${cue}`)}<p>${cue}</p></div>`).join("")}</div><small>Blue cadet = example position · L/R = left/right foot · yellow dot = pivot · diagrams are schematic, not to scale</small></figure>`;
}
